"""
Parallel AEGIS Trading Pipeline — clean implementation.

Loaded via ``parallel_full_pipeline_clean.py`` (repo root shim) or
``python -m backend.src.cli``. Dotenv and ``sys.path`` are configured from the
repository root before importing other ``backend`` modules.
"""
...
def _repo_root() -> Path:
    """Hedgera repo root (parent of ``backend/``)."""
...
async def run_parallel_clean_main(argv: List[str] | None = None) -> None:
    """Entry point for the parallel clean pipeline (CLI or shim)."""
...
def main_sync(argv: List[str] | None = None) -> None:
    asyncio.run(run_parallel_clean_main(argv))
